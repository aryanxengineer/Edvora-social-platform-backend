...
export class FollowRepository {
  async create(followerId: string, followingId: string, session?: mongoose.ClientSession) {
    return FollowModel.create([{ followerId, followingId }], { session }).then(r => r[0]);
  }

  async delete(followerId: string, followingId: string, session?: mongoose.ClientSession) {
